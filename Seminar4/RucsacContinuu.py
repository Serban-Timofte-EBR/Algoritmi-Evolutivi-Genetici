...
def mutatie_populatie(pop, dim, n, c, v, cmax, probabilitate_mutatie, sigma):
    ...
    for i in range(dim):
        individ = pop[i][:n].copy   #excludem ultima coloana cu fitness-ul => extragem genele
        ...
        if(cost):
            individ = individ + [fitness]
            pop_muntanta[i] = individ.copy()
    return pop_muntanta

#How to run the code? Follow these instructions:
'''
import RucsacContinuu
pop, dim, n , c, v, cmax = RucsacContinuu.gen(30,10)
pop_m = RucsacContinuu.mutatie_populatie(pop, dim, n, c, v, cmax, 0.2, 0.25)
import numpy
pop = numpy.asarray(pop)
pop_m = numpy.asarray(pop_m)
'''

pop, dim, n , c, v, cmax = gen(30,10)
pop_m = mutatie_populatie(pop, dim, n, c, v, cmax, 0.2, 0.25)
pop = numpy.asarray(pop)
pop_m = numpy.asarray(pop_m)